...
-- MIDI globals initialization
midi_data = {}
midi_available = false
midi_enabled = false

-- MIDI Clock globals initialization
midi_beat = 1
midi_bar = 1
midi_tick = 0
midi_new_beat = false
midi_time_numerator = 4
midi_time_denominator = 4
midi_transport_playing = false
midi_bpm = 120.0

-- MIDI Clock trigger flags (one-frame events)
midi_beat_trigger = false
midi_bar_trigger = false
...
-- EYESY BOILERPLATE INTEGRATION
-- All boilerplate functionality is now part of the main eyesy system
...
midiButton1Active = false
midiButton2Active = false
midiButton3Active = false
midiButton4Active = false
midiPedalActive = false

midiButton1BeatCount = 0
midiButton2BeatCount = 0
midiButton3BeatCount = 0
midiButton4BeatCount = 0
midiPedalBeatCount = 0

totalBeats = 0
beatCount = 0

-- Previous button states for detecting state changes
local midiButton1PrevState = false
local midiButton2PrevState = false
local midiButton3PrevState = false
local midiButton4PrevState = false
local midiPedalPrevState = false
...
function createBandpassBiquad(centerFreq, bandwidth, sampleRate)
    local omega = 2 * math.pi * centerFreq / sampleRate
    local sin_omega = math.sin(omega)
    local cos_omega = math.cos(omega)
    local alpha = sin_omega * 0.5 * bandwidth

    local b0 = alpha
    local b1 = 0
    local b2 = -alpha
    local a0 = 1 + alpha
    local a1 = -2 * cos_omega
    local a2 = 1 - alpha

    b0, b1, b2 = b0/a0, b1/a0, b2/a0
    a1, a2 = a1/a0, a2/a0

    return {b0=b0, b1=b1, b2=b2, a1=a1, a2=a2, x1=0, x2=0, y1=0, y2=0}
end

function processBiquad(filter, input)
    local output = filter.b0 * input + filter.b1 * filter.x1 + filter.b2 * filter.x2
...
function processAudioBuffers()
    ...
    for i = 1, 256 do
        local sampleL = inL[i] or 0
        local sampleR = inR[i] or 0

        local filteredL = processBiquad(filterL, sampleL)
        local filteredR = processBiquad(filterR, sampleR)
        local filteredMono = (filteredL + filteredR) / 2

        audioL_filtered[i] = filteredL * (3 - math.exp(-math.abs(filteredL)))
        audioR_filtered[i] = filteredR * (3 - math.exp(-math.abs(filteredR)))
        audioMono_filtered[i] = filteredMono * (3 - math.exp(-math.abs(filteredMono)))
    end

    local sumL, sumR, sumMono = 0, 0, 0
    for i = 1, 32 do
        sumL = sumL + math.abs(audioL_filtered[i] or 0)
        sumR = sumR + math.abs(audioR_filtered[i] or 0)
        sumMono = sumMono + math.abs(audioMono_filtered[i] or 0)
    end

    avgAudioL = sumL / 32
    avgAudioR = sumR / 32
    avgAudioMono = sumMono / 32
end
...
function getAudioSample(channel, index, default)
    default = default or 0
    if channel == "L" then
        return audioL_filtered[index] or default
    elseif channel == "R" then
        return audioR_filtered[index] or default
    elseif channel == "mono" then
        return audioMono_filtered[index] or default
    end
    return default
end
...
function colorPickHsb(knob, name)
    local k6 = (knob * 5) + 1
    local hue = (k6 * 255) % 255
    local kLow = math.min(knob, 0.49) * 2
    local kLowPow = math.pow(kLow, 2)
    local kH = math.max(knob, 0.5) - 0.5
    local kHigh = 1 - (kH * 2)
    local kHighPow = math.pow(kHigh, 0.5)

    local bright = kLow * 255
    local sat = kHighPow * 255

    name:setHsb(hue, sat, bright)
end

-- MIDI PROCESSING FUNCTIONS
function processMIDI()
    -- MIDI beat detection with division conditions
    if midi_beat_trigger then
        totalBeats = totalBeats + 1
        beatCount = beatCount + 1

        -- Beat division conditions using MIDI clock (24 PPQN)
        midiButton1BeatCount = beatCount % 3
        midiButton2BeatCount = beatCount % 1
        midiPedalBeatCount = beatCount % 4
    end

    -- Sub-beat divisions using MIDI tick (24 PPQN)
    if midi_enabled and midi_tick then
        midiButton3BeatCount = midi_tick % 8
        midiButton4BeatCount = midi_tick % 4
    end

    -- Regular MIDI message handling with safe note state tracking
    if midi_enabled and midi_available and midi_data then
        local status = midi_data[1] or 0
        local channel = midi_data[2] or 0
        local pitch = midi_data[3] or 0
        local velocity = midi_data[4] or 0

        -- Handle Note ON/OFF messages
        if (status >= 144 and status <= 159) or (status >= 128 and status <= 143) then
            if noteStates[pitch] ~= nil then
                noteStates[pitch] = (status >= 144 and status <= 159 and velocity > 0)
            end
        end

        -- Update button states from note states
        midiButton1Active = noteStates[4]
        midiButton2Active = noteStates[5]
        midiButton3Active = noteStates[6]
        midiButton4Active = noteStates[7]
        midiPedalActive = noteStates[15]

        -- Update previous states for change detection
        midiButton1PrevState = midiButton1Active
        midiButton2PrevState = midiButton2Active
        midiButton3PrevState = midiButton3Active
        midiButton4PrevState = midiButton4Active
        midiPedalPrevState = midiPedalActive
    end
end
...
function initBoilerplate()
    ...
    local sampleRate = 11025
    local centerFreq = 95
    local bandwidth = 0.1

    filterL = createBandpassBiquad(centerFreq, bandwidth, sampleRate)
    filterR = createBandpassBiquad(centerFreq, bandwidth, sampleRate)
